<?php
     ...
     */
     ...
     public function getMostUsedVariantForStation(int $stationId, string $date)
     {
        $start = $date.' 00:00:00';
        $end = $date.' 23:59:59';

        $sql = "SELECT pv.variant_id, SUM(ps.units) AS total_units, pv.variant_description
        FROM palletize_session ps
         JOIN pallet p ON p.pallet_id = ps.pallet_id
         JOIN tote t
         ...
         $variant = $this->selectOne($sql, ["stationId"=> $stationId, 'start'=> $start, "end"=>$end]);
    ...
    public function getLatestTeamForStation(int $stationId, string $date)
     {
        // $sql = "SELECT station_id, station_name FROM station";

        // $station = $this->selectAll($sql);
        ...
        $result = $this->selectOne($sql, ['stationId' => $stationId, 'date' => $date]);
    ...
    public function getStationTodayKPI(int $stationId, string $date)
    {
        ...
        $variant = $this->getMostUsedVariantForStation($stationId, $date);
        ...
        if($variant)
        {
            $summary = $this->getProductSummary($variant['variant_id'], $date);
